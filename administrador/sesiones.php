...
$datepickerini = date('Y-m-d', strtotime($_GET['datepickerini']));
$datepickerfin = date('Y-m-d', strtotime($_GET['datepickerfin']));
...
foreach($casilla as $hora)
{
	...
	$rango_total=0;
	$rango_disponibles=0;
	for($x=strtotime($datepickerini); $x<=strtotime($datepickerfin); $x+=86400)
	{
		$dia = date('Y-m-d', $x);
